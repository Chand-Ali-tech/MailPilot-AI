...
async def get_latest_emails(
    ...
    """
    Fetches the user's latest Gmail messages using their stored Google OAuth connection.
    ...
    # 1. Get current access token (or refresh if not present)
    access_token = connection.access_token
    if not access_token:
        access_token = await refresh_google_access_token(connection, session)

    async with httpx.AsyncClient() as client:
        # 2. Fetch list of recent message IDs
        headers = {"Authorization": f"Bearer {access_token}"}
        list_url = f"{GMAIL_API_BASE_URL}/messages?maxResults={limit}"

        res = await client.get(list_url, headers=headers)

        # Handle token expiration (401) by refreshing and retrying once
        ...
        if res.status_code != 200:
            raise HTTPException(
                ...
                detail=f"Failed to fetch emails from Gmail API: {res.text}",
            )
        ...
        for msg in raw_messages:
            ...
            msg_res = await client.get(detail_url, headers=headers)
